...
test_that("Test s-bootstrap.R on data set #1", {
	...
	start <- Sys.time()
	GEV <- s_bootstrap(ams, distributions$GEV, "L-moments")
	end <- Sys.time()
	# print(end - start)
	...
	GUM <- s_bootstrap(ams, distributions$GUM, "L-moments")
	...
	NOR <- s_bootstrap(ams, distributions$NOR, "L-moments")
	...
	LNO <- s_bootstrap(ams, distributions$LNO, "L-moments")
	...
	GLO <- s_bootstrap(ams, distributions$GLO, "L-moments")
	...
	PE3 <- s_bootstrap(ams, distributions$PE3, "L-moments")
	...
	GNO <- s_bootstrap(ams, distributions$GNO, "L-moments")
	...
	WEI <- s_bootstrap(ams, distributions$WEI, "L-moments")
	...
	GPA <- s_bootstrap(ams, distributions$GPA, "L-moments")
...
test_that("Test s-bootstrap.R on data set #2", {
	...
	# Log-Pearson Type III (LP3) Distribution
	LP3 <- s_bootstrap(ams, distributions$LP3, "L-moments")
