...
To compute the $R^2$ statistic, we perform a linear regression of the streamflow data against the predictions of the parametric model.
The $R^2$ statistic describes how well the parametric model captures variance in the streamflow data. 
Higher is better.

### RMSE - Root-Mean Squared Error

The `RMSE` statistic describes the average *absolute* difference between the data and the predictions of the parametric model.
Lower is better.
